angular.module('app').controller('PAGCtrl', function (AuthVerify, $scope, $http, $state, $stateParams, $location, $window) {
    $(".last-col").hide();
    ...
    // 公共账户组树
    $scope.GetPublicUserTree = function () {
        TreesType = 1;
        function  GetRootNode() {
            var nodeid = "00000000-0000-0000-0000-000000000000";
            $http({
                method: 'GET',
                url: storage.getItem("InterfaceUrl") + 'TreeNode.ashx?op=GetPublicUserTree&accessToken=' + storage.getItem("userAdminToken") + '&NodeID=' + nodeid,
                ...
                if (data.result != "false") {
                    ...
                    var setting = {
                        async: {
                            enable: true,
                            url: getPublicUserTreeUrl,
                            dataFilter: ajaxPublicUserTreeDataFilter
                        },
                        view: {
                        ...
                        callback: {
                            onClick: PublicUserTreeOnClick,
                            onRightClick: PublicUserTreeOnRightClick,
                            onAsyncSuccess: PublicUserTreeOnAsyncSuccess,
                            onAsyncError: PublicUserTreeOnAsyncError
                        }
                    };
                    PublicUserTree = $.fn.zTree.init($("#PublicUserTree"), setting, obj);//初始化
                } else {
        ...
        function getPublicUserTreeUrl(treeId, treeNode) {
            TreesType = 1;
            var nodeid = "";
            ...
            var Url = storage.getItem("InterfaceUrl") + 'TreeNode.ashx?op=GetPublicUserTree&accessToken=' + storage.getItem("userAdminToken") + '&NodeID=' + nodeid;
            return Url;
        }

        function ajaxPublicUserTreeDataFilter(treeId, parentNode, responseData) {
        ...
        function PublicUserTreeOnAsyncSuccess(event, treeId, treeNode, msg) {
            if (currentNodeID != '') {
                for (var i = 0; i < treeNode.children.length; i++) {
                    if (treeNode.children[i].id == currentNodeID) {
                        PublicUserTree.selectNode(treeNode.children[i]);
                    }
                }
            }
        }

        function PublicUserTreeOnAsyncError(event, treeId, treeNode, XMLHttpRequest, textStatus, errorThrown) {
            alert(XMLHttpRequest);
        };

        function PublicUserTreeOnClick(event, treeId, treeNode) {
            $(".ztree li a").css({ "padding-left": "4px", "margin-left": "-4px" });
            treeSelectNodeID = treeNode.id;
            treeSelectNode = treeNode;
            PublicUserTree.selectNode(treeNode);
            PublicUserTree.expandNode(treeNode);
        ...
        function PublicUserTreeOnRightClick(event, treeId, treeNode) {
            $("#rMenu ul").show();
            HideRMenu();
            if (!treeNode == "") {
                if (treeNode.Type == 1) {//OU
                ...
                //document.getElementById("PublicUserTree").scrollTop = storage.getItem("scrollTop");
            }
            else {
                //alert("请选择节点！");
            }
        ...
        $scope.SearchPublicUserTree = function () {
            currentNodeID = '';
            $(".last-col").hide();
            if ($scope.seachNode == "" || $scope.seachNode == undefined || $scope.seachNode == null) {
                $scope.GetPublicUserTree();
            } else {
                var searchValue = $.trim($scope.seachNode);
                var myUrl = storage.getItem("InterfaceUrl") + 'TreeNode.ashx?op=SearchPublicUserTree&accessToken=' + storage.getItem("userAdminToken") + '&Searchstr=' + encodeURI(searchValue);
                    ...
                    function (response) {
                        ...
                        if (response.result != "false") {
                            ...
                            var setting = {
                                ...
                                callback: {
                                    onClick: PublicUserTreeOnClick, //点击节点时 回调,
                                    onRightClick: PublicUserTreeOnRightClick,
                                    onAsyncSuccess: PublicUserTreeOnAsyncSuccess,
                                    onAsyncError: PublicUserTreeOnAsyncError
                                }
    ...
    function refreshNode() {
        console.log("dfdfd");
        var type = "refresh", silent = false, nodes = PublicUserTree.getSelectedNodes();
        PublicUserTree.selectNode(nodes);
        PublicUserTree.expandNode(nodes);
        PublicUserTree.reAsyncChildNodes(nodes[0], type, silent);
    }

    function refreshParentNode() {
        var type = "refresh", silent = false, nodes = PublicUserTree.getSelectedNodes();
        var parentNode = PublicUserTree.getNodeByTId(nodes[0].parentTId);
        PublicUserTree.selectNode(parentNode);
        PublicUserTree.expandNode(parentNode);
        PublicUserTree.reAsyncChildNodes(parentNode, type, silent);
    }
    ...
    $scope.AddOU = function () {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=AddOu&accessToken=' + storage.getItem("userAdminToken"),
            ...
            if (callbackObj.result == "true") {
                PublicUserTree.refresh();
                var parentNodes = PublicUserTree.getNodesByParam("id", treeSelectNodeID, null);
                if (!parentNodes[0].open) {
                    PublicUserTree.expandNode(parentNodes[0], true, true, true);
                    PublicUserTree.reAsyncChildNodes(parentNodes[0], "refresh");
                } else {
                    PublicUserTree.reAsyncChildNodes(parentNodes[0], "refresh");
                }
    ...
    $scope.ModifyOuPanel = function (ouid) {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=GetOuInfo&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            var callbackObj = response.data;
            if (callbackObj.result == "true") {
    ...
    $scope.ModifyOu = function () {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=ModifyOu&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            angular.element(".preloader").css("display", "none");
            var callbackObj = response.data;
            ErrorHandling(callbackObj.result, callbackObj.errCode, callbackObj.errMsg);
            if (callbackObj.result == "true") {
                var parentNodes = PublicUserTree.getNodesByParam("id", $scope.ModifyOuID, null)[0].getParentNode();
                if (parentNodes != null) {
                    if (!parentNodes.open) {
                        PublicUserTree.expandNode(parentNodes, true, true, true);
                        PublicUserTree.reAsyncChildNodes(parentNodes, "refresh");
                    } else {
                        PublicUserTree.reAsyncChildNodes(parentNodes, "refresh");
                    }
                    ...
                    treeSelectNode.Name = $scope.OUName;
                    PublicUserTree.updateNode(treeSelectNode);
                }
            }
        }, function errorCallback(e) {
    ...
    $scope.MoveOu = function () {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=MoveOu&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            angular.element(".preloader").css("display", "none");
            var callbackObj = response.data;
    ...
    $scope.DeleteOu = function () {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=DeleteOu&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            var callbackObj = response.data;
            if (callbackObj.result == "true") {
                ...
                currentNodeID = '';
                var parentNodes = PublicUserTree.getNodesByParam("id", $scope.DeleteOuID, null)[0].getParentNode();
                if (parentNodes != null) {
                    if (!parentNodes.open) {
                        PublicUserTree.expandNode(parentNodes, true, true, true);
                        PublicUserTree.reAsyncChildNodes(parentNodes, "refresh");
                    ...
                    PublicUserTree.selectNode(parentNodes);
                    $scope.ModifyOuPanel(parentNodes.id);
                } else {
                    $(".last-col").hide();
                    $scope.SearchPublicUserTree();
                }
            } else {
    ...
    $scope.AddGroup = function () {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=AddGroup&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            angular.element(".preloader").css("display", "none");
            var callbackObj = response.data;
            ...
            if (callbackObj.result == "true") {
                ...
                $scope.ModifyGroupPanel(callbackObj.data.GroupID);
            }
        }, function errorCallback(e) {
    ...
    $scope.ModifyGroupPanel = function (groupid) {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=GetGroupInfo&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            var callbackObj = response.data;
            if (callbackObj.result == "true") {
    ...
    $scope.ModifyGroup = function () {
        ...
        var paramObj = {
            ...
            "Admins": $scope.GroupAdmins,
            "Description": $scope.GroupDesc,
        }
        angular.element(".preloader").css("display", "block");
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=ModifyGroup&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            angular.element(".preloader").css("display", "none");
            var callbackObj = response.data;
            ErrorHandling(callbackObj.result, callbackObj.errCode, callbackObj.errMsg);
            if (callbackObj.result == "true") {
                var parentNodes = PublicUserTree.getNodesByParam("id", treeSelectNodeID, null)[0].getParentNode();
                if (parentNodes != null) {
                    if (!parentNodes.open) {
                        PublicUserTree.expandNode(parentNodes, true, true, true);
                        PublicUserTree.reAsyncChildNodes(parentNodes, "refresh");
    ...
    $scope.DeleteGroup = function () {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=DeleteGroup&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            var callbackObj = response.data;
            if (callbackObj.result == "true") {
    ...
    $scope.AddUser = function () {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=AddUser&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            angular.element(".preloader").css("display", "none");
            var callbackObj = response.data;
    ...
    $scope.ModifyUserPanel = function (userid, op) {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=GetUserInfo&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            var callbackObj = response.data;
            if (callbackObj.result == "true") {
    ...
    $scope.ChangeUser = function (op) {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=ChangeUser&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            angular.element(".preloader").css("display", "none");
            var callbackObj = response.data;
            ...
            if (callbackObj.result == "true") {
                if (op == 'add') {
                    ...
                    $scope.editUserStep = 1;
                    $scope.gotoTab(4);
                    var parentNodes = PublicUserTree.getNodesByParam("id", $scope.ModifyUserID, null)[0].getParentNode();
                    if (parentNodes != null) {
                        if (!parentNodes.open) {
                            PublicUserTree.expandNode(parentNodes, true, true, true);
                            PublicUserTree.reAsyncChildNodes(parentNodes, "refresh");
    ...
    $scope.MoveUser = function () {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=MoveNodes&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            angular.element(".preloader").css("display", "none");
            var callbackObj = response.data;
    ...
    $scope.GetUserExchangeInfo = function () {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=GetUserExchangeInfo&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            var callbackObj = response.data;
            if (callbackObj.result == "true") {
    ...
    $scope.ChangeUserExchangeInfo = function () {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=ChangeUserExchangeInfo&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            angular.element(".preloader").css("display", "none");
            var callbackObj = response.data;
    ...
    $scope.ChangeUserMemberof = function (op) {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=ChangeUserMemberof&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            angular.element(".preloader").css("display", "none");
            var callbackObj = response.data;
    ...
    $scope.EnableUser = function () {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=EnableUser&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            var callbackObj = response.data;
            ErrorHandling(callbackObj.result, callbackObj.errCode, callbackObj.errMsg);
            if (callbackObj.result == "true") {
                var node = PublicUserTree.getNodesByParam("id", $scope.ModifyUserID, null);
                node[0].Status = true;
                node[0].iconSkin = 'Nuser';
                PublicUserTree.updateNode(node[0]);
                $scope.ModifyUserPanel($scope.ModifyUserID, 'edit');
            }
        }, function errorCallback(e) {
    ...
    $scope.DisableUser = function () {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=DisableUser&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            var callbackObj = response.data;
            if (callbackObj.result == "true") {
    ...
    $scope.ResetPassword = function () {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=ResetUserPassword&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            var callbackObj = response.data;
            if (callbackObj.result == "true") {
    ...
    $scope.DeleteUser = function () {
        ...
        $http({
            method: 'post',
            url: storage.getItem("InterfaceUrl") + 'PublicUsers.ashx?op=DeleteUser&accessToken=' + storage.getItem("userAdminToken"),
            data: JSON.stringify(paramObj)
        }).then(function successCallback(response) {
            var callbackObj = response.data;
            if (callbackObj.result == "true") {
        ...
        if (ischeck) {
            ...
            for (var i = 0; i < range; i++) {
                if (i == 6 && (!str.match(/\d+/g))) {
                    ...
                    str += arr[pos];
                } else if (i == 7 && !regSpecialCharacter.test(str)) {
                    var arr2 = ['!', '@', '#', '$', '%', '*', '(', ')', '.'];
