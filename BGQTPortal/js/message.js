...
|---------------------------------------------------------------------------------------*/
MessageBox = function(element, id, message, left, top, timeout, width, scroll)
{
	// Init value
	this.timeout = -1;	// ms
	this.message = "";
	...
	this.id = id;
	if (!isNaN(left) && left == left) this.marginLeft = parseInt(left);
	if (!isNaN(top) && top == top) this.marginTop = parseInt(top);
	if (!isNaN(width) && width == width) this.width = parseInt(width);
...
MessageBox.prototype = {
	...
	Show : function()
	{
		...
		//megbox.style.marginTop = "8px";//(node_top + node_view.height + this.top) + "px";
		//megbox.style.marginLeft = 0; //(node_left + this.left) + "px";
