angular.module('app').controller('hdbCtrl', function (AuthVerify, $scope, $http, $state, $stateParams, $location, $window) {
 ...
 if ($window.innerWidth >= 1024) {
    ...
    $.getJSON(storage.getItem("InterfaceUrl") + 'SystemReport.ashx?op=GetSystemUserCount&accessToken=' + storage.getItem("userAdminToken"), function (json) {
        if (json.result == "true") {
            $scope.UserCounts = json.data.Conut;
        }
    });
