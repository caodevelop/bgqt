angular.module('app').controller('NLCtrl', function (AuthVerify, $scope, $http, $state, $stateParams, $location, $window) {
    $scope.StartTime = '';
    $scope.EndTime = '';
    $scope.Searchstr = '';
    $scope.pSize = 10;
    ...
    //获取 MailDB 列表
    $scope.GetNoLoginUsers = function (currentPage) {
        $scope.currentPage = currentPage;
        $scope.Lists = [];
        $http({
            method: 'GET',
            url: storage.getItem("InterfaceUrl") + 'SystemReport.ashx?op=GetNoLoginUsers&accessToken=' + storage.getItem("userAdminToken") + '&PageSize=' + $scope.pSize + '&CurPage=' + currentPage + '&Searchstr=' + encodeURI($scope.Searchstr),
    ...
    $scope.pageChanged = function (currentPage) {
        $scope.GetNoLoginUsers(currentPage);
    };
    $scope.clearSearch = function () {
        $scope.StartTime = '';
        $scope.EndTime = '';
        $scope.Searchstr = '';
        $scope.GetNoLoginUsers(1);
    }
    ...
    $scope.ExportNoLoginUsersToExcel = function () {
        $('#Export').attr('href', storage.getItem("InterfaceUrl") + 'SystemReport.ashx?op=ExportNoLoginUsersToExcel&accessToken=' + storage.getItem("userAdminToken") + '&PageSize=10000&CurPage=1&Searchstr=' + $scope.Searchstr + '&StartTime=' + $scope.StartTime + '&EndTime=' + $scope.EndTime); 
    }
});