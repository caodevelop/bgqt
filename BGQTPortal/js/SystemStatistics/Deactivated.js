angular.module('app').controller('DeactivatedCtrl', function (AuthVerify, $scope, $http, $state, $stateParams, $location, $window) {
    $scope.Searchstr = '';
    $scope.pSize = 10;
    $scope.maxSize = 5;
    $scope.Lists = [];
    ...
    //获取 MailDB 列表
    $scope.GetDisableUsers = function (currentPage) {
        $scope.currentPage = currentPage;
        $scope.Lists = [];
        $http({
            method: 'GET',
            url: storage.getItem("InterfaceUrl") + 'SystemReport.ashx?op=GetDisableUsers&accessToken=' + storage.getItem("userAdminToken") + '&PageSize=' + $scope.pSize + '&CurPage=' + currentPage + '&Searchstr=' + encodeURI($scope.Searchstr),
    ...
    $scope.pageChanged = function (currentPage) {
        $scope.GetDisableUsers(currentPage);
    };
    $scope.clearSearch = function () {
        $scope.Searchstr = '';
        $scope.GetDisableUsers(1);
    }
    ...
    $scope.ExportDisableUsersToExcel = function () {
        $('#Export').attr('href', storage.getItem("InterfaceUrl") + 'SystemReport.ashx?op=ExportDisableUsersToExcel&accessToken=' + storage.getItem("userAdminToken") + '&PageSize=10000&CurPage=1&Searchstr=' + $scope.Searchstr);
    }
});