    ...
    public class SendNoticeMessage
    {
        ...
        private void BuildingEntryAndLeaveSystemMailBody(string body, List<UserInfo> nlist, List<UserInfo> llist, out string mailbody)
        {
            ...
            if (nlist.Count > 0)
            {
                ...
                mailbody += "<table><tr><td style='width:200px;'>员工号</td><td style='width:200px;'>姓名</td><td style='width:200px;'>手机号码</td><td style='width:400px;'>AD账户</td><td style='width:200px;'>创建时间</td></tr>";
                foreach (var m in nlist)
                {
                    mailbody += "<tr><td>" + m.EMPLID + "</td><td>" + m.NAME + "</td><td>" + m.PHONE1 + "</td><td>" + m.userPrincipalName + "</td><td>" + m.Entry_DT + "</td></tr>";
                }
                mailbody += "</table><br>";
            }
            if (llist.Count > 0)
            {
                ...
                mailbody += "<table><tr><td style='width:200px;'>员工号</td><td style='width:200px;'>姓名</td><td style='width:200px;'>手机号码</td><td style='width:400px;'>AD账户</td><td style='width:200px;'>离职时间</td></tr>";
                foreach (var m in llist)
                {
                    mailbody += "<tr><td>" + m.EMPLID + "</td><td>" + m.NAME + "</td><td>" + m.PHONE1 + "</td><td>" + m.userPrincipalName + "</td><td>" + m.TERMINATION_DT + "</td></tr>";
                }
                mailbody += "</table>";
            }
        }
